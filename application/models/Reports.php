        ...
        if ($report_type) {
            ...
            if ($params) {
                ...
                if ($report_type == 'dar' && $params['dar_status']) {
                    ...
                    if ($params['dar_status'] === 'x') {
                        ...
                        $where.=' WHERE A.STATUS IS NULL OR A.STATUS = ""';
            ...
            switch ($report_type) {
                ...
                case 'dar':
                    ...
                    CONCAT(B.EMP_LNAME,' ',B.EMP_FNAME)AS 'EMP_NAME',B.GRP_CODE,B.COMP_CODE
                    FROM nets_emp_activity AS A
                        ...
                        LEFT JOIN (SELECT GROUP_CONCAT(CONCAT(VISIT_FNAME,' ',VISIT_LNAME) SEPARATOR ', ') AS `VISITORS`, ACTIVITY_ID
                ...
                case 'ehc':
                    ...
                    IF(A.A4 = 'Y','Yes','NO') AS `A4`,IF(A.A5 = 'Y','Yes','NO') AS `A5`,A.A4WHERE,A.A4WHEN,A.A5WHEN,A.EMP_CODE,B.COMP_CODE,B.GRP_CODE,A.RUSHNO,A.REASON,B.EMP_FNAME,B.EMP_LNAME,
                    define_rush(A.STATUS) AS 'STATUS',
                    CONCAT(B.EMP_LNAME,' ',B.EMP_FNAME) AS 'EMP_NAME'
                ...
                case 'visitors':
                    $queryString = "SELECT A.VISITOR_ID,A.VISIT_DATE,A.CHECKIN_TIME,A.CHECKOUT_TIME,A.COMP_NAME,A.COMP_ADDRESS,A.EMAIL_ADDRESS,A.MOBILE_NO,A.TEL_NO,A.RES_ADDRESS,A.VISIT_PURP,A.A1,convert_sickness(A.A2) as `A2`,A.A2DATES,A.A3TRAVEL_DATES,A.A3PLACE,A.A3RETURN_DATE,
                    CONCAT(B.EMP_FNAME,' ',B.EMP_LNAME) AS `EMP_NAME`,B.COMP_CODE,B.GRP_CODE,B.EMP_CODE,
                    CONCAT(A.VISIT_FNAME,' ',A.VISIT_LNAME) AS `VISIT_NAME`,
                    IF(A.A3 = 1,'Yes','No') AS `A3`,
                    define_status(A.STATUS) AS `_status`
                ...
                case 'hdf':
                    ...
                    $queryString="SELECT B.EMP_LNAME,B.EMP_FNAME,B.AGE,B.GENDER,B.CIVIL_STAT,B.PRESENT_PROV,B.PRESENT_ADDR1,B.TEL_NO,B.MOBILE_NO,C.COMP_NAME,
                    ...
                    G.A1PLACE AS `ga1place`,G.A1RETURN_DATE AS `ga1return_date`,IF(G.A2 = 'Y','Yes','No') AS `ga2`,G.A2TRAVEL_DATES AS `ga2travel_date`,G.A2PLACE AS `ga2place`,G.A2RETURN_DATE AS `ga2return_date`,IF(G.A3 = 'Y','Yes','No') AS `ga3`,G.A3CONTACT_DATE AS `contact_date`,IF(G.A4 = 'Y','Yes','No') AS `ga4`,G.A4NAME AS `gname`,
                    ...
                    H.A6,H.A7,GROUP_CONCAT(J.DISEASE SEPARATOR ', ') as `disease`,
                    A.HDF_DATE,A.HDF_ID,A.EMP_CODE as `EMP_CODE`,B.COMP_CODE as `COMP_CODE`,B.GRP_CODE as `GRP_CODE`
