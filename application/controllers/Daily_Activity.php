    ...
    public function daily_activities_list() {
       $table = "
         (
            ...
            ACT.ACTIVITY_ID,
              ...
              ACT.LOCATION, 
             CASE 
                WHEN (SELECT MAX(NVL(PP.STATUS, 'ZZZ')) FROM NETS_EMP_ACT_PARTICIPANTS PP
     ...
     public function pending_confirmation_list() {
        ...
        $table = "(
            ...
            OR (ISNULL(PART.STATUS) AND PART.EMP_CODE = '".$this->session->userdata('EMP_CODE')."' AND (ACT.STATUS <> 'DENIED' AND ACT.STATUS <> 'CANCELLED'))
                ...
                if($employee != $host_employee) {
                    if($current_status != 'DONE') {
                        ...
                        if($host_employee) {
                            ...
                            // SEND SMS/ EMAIL TO VISITORS AND HOST EMPLOYEE
