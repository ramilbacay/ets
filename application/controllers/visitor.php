...
class visitor extends CI_Controller {
	...
	public function add_visitor_log(){
		...
		if($this->input->is_ajax_request() && $this->input->post()){
			...
			//$this->form_validation->set_rules('dateOfVisit','Date of Visit','required|trim');
			...
			/* $this->form_validation->set_rules('controlSelectStatusQ1','Sickness/symptons','required|trim'); */
			...
			if($this->form_validation->run() == TRUE){
				...
				/* For inserting employee activity */
					$insert_employee_activity = array(
						...
						'ACTIVITY_TYPE' => '',
						/* 'LOCATION' => $this->input->post('inputCompanyAdd'), */
	...
	public function update_visitor_logs(){
		...
		if($this->input->is_ajax_request() && $this->input->post()){
			...
			if($this->form_validation->run() == TRUE){
				...
				$this->input->post('EditinputStartEndDate') , $this->input->post('EditcontrolSelectStatusQ2') , $this->input->post('EditinputTravelDate') , $this->input->post('EditinputTravelLoc') ,  $this->input->post('EditinputDatOfReturn') , $this->session->userdata('EMP_CODE') , date('Y-m-d') , $id);

				if($update_visitory_logs){
					$activity_id = $this->visitor_model->getActivityID($id);
					$update_acitivy = $this->visitor_model->update_activity($activity_id->ACTIVITY_ID , $this->input->post('EditinputTimeFrom') , $this->input->post('EditinputTimeTo'));
				}
				...
				if($update_visitory_logs){
					...
					foreach ($this->input->post('EditinputPart') as $value) {
						$act_part = array(
							'ACTIVITY_ID' => $visitor_activity_id->ACTIVITY_ID,
							'EMP_CODE' => $value,
							'STATUS' => 'For Confirmation',
							'STATUS_DATE' => date('Y-m-d'),
							'UPDATE_USER' => $this->session->userdata('EMP_CODE'),
	...
	public function view_visitor_logs() {
        if($this->input->is_ajax_request()) {
			...
			$participants = $this->visitor_model->get_visitor_part($this->input->get('id'));
			$Q1 = $this->visitor_model->getQuestionaire('EHC', 3);
			$Q2 = $this->visitor_model->getQuestionaire('EHC', 5);
			$employee = $this->visitor_model->getEmployee();
	...
	public function search_CheckOut() {
		$table = "
		  ...
		  WHERE nets_emp_activity.STATUS != '' AND nets_emp_activity.STATUS != 'CANCELLED' AND nets_visit_log.STATUS != 'DONE' AND nets_emp_activity.STATUS != 'DENIED' AND nets_visit_log.MEETING_ID != ''
		  ";

		  if($this->input->get('vistior_name')) {
			$table .= " AND nets_visit_log.VISIT_LNAME LIKE '%".$this->input->get('vistior_name')."%' Or nets_visit_log.VISIT_FNAME LIKE '%".$this->input->get('vistior_name')."%'";
		  }
		  ...
		  if($this->input->get('status') != 0){
				if($this->input->get('status') == 1){
					$table .= "  AND nets_emp_activity.STATUS = ''";
				}else if($this->input->get('status') == 2){
					$table .= " AND nets_visit_log.MEETING_ID IS NOT NULL AND nets_visit_log.CHECKIN_TIME IS NULL";
	...
	public function search_visitor() {
		$table = "
		  (SELECT CONCAT(VISIT_LNAME , ', ' , VISIT_FNAME , ' '  , VISIT_MNAME) as FULL_NAME ,
		  CONCAT(nets_emp_info.EMP_LNAME , ', ' , nets_emp_info.EMP_FNAME , ' '  , nets_emp_info.EMP_MNAME) as VISIT_FULL_NAME ,
		  ...
		  if($this->input->get('vistior_name')) {
			$table .= " AND VISIT_LNAME LIKE '%".$this->input->get('vistior_name')."%' Or VISIT_FNAME LIKE '%".$this->input->get('vistior_name')."%'";
		  }
		 ...
		 $columns = array(
			 ...
			 array( 'db' => 'VISIT_DATE', 'dt' => 1, 'field' => 'VISIT_DATE' ),
			 array( 'db' => 'CHECKIN_TIME',  'dt' => 2, 'field' => 'CHECKIN_TIME' ),
			 array( 'db' => 'CHECKOUT_TIME_HOST',   'dt' => 3, 'field' => 'CHECKOUT_TIME_HOST' ),
			 array( 'db' => 'FULL_NAME', 'dt' => 4, 'field' => 'VISIT_LNAME'),
